...
static QByteArray _rel_to_abs(CSYNC* ctx, const QByteArray &relativePath) {
    QByteArray abs = QByteArray::fromRawData(ctx->local.uri, strlen(ctx->local.uri));
    abs.reserve(abs.size() + 1 + relativePath.size());
    abs += '/';
    abs += relativePath;
    return abs;
}
...
out:
  ...
  switch (ctx->current) {
    case LOCAL_REPLICA:
      ctx->local.files.emplace(path, std::move(fs));
      break;
    case REMOTE_REPLICA:
      ctx->remote.files.emplace(path, std::move(fs));
      break;
    default:
      break;
  }

...
struct DiscoveryDirectoryResult
{
    ...
    QString msg;
    int code;
    std::vector<std::unique_ptr<csync_file_stat_t>> list;
    unsigned listIndex;
...
public:
    ...
    void start();
    void abort();
    std::vector<std::unique_ptr<csync_file_stat_t>> &&takeResults() { return std::move(_results); }
...
private:
    std::vector<std::unique_ptr<csync_file_stat_t>> _results;
    QString _subPath;
